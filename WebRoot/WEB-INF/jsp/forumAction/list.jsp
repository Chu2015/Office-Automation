...
<div id="MainArea">
    <table cellspacing="0" cellpadding="0" class="TableStyle">
        ...
        <s:iterator value="#forumList" status="status">
			<tr class="TableDetail1 template">
				...
				<td>${description}&nbsp;</td>
				<td>
					<s:a action="forum_delete?id=%{id}" onclick="return delConfirm()">删除</s:a>
					<s:a action="forum_editUI?id=%{id}">修改</s:a>
					
					<s:if test="#status.first == true">
						<span class="disabled">上移</span>
					</s:if >
					<s:else>
						<s:a action="forum_moveUp?id=%{id}">上移</s:a>
					</s:else>
					...
					<s:else>
						<s:a action="forum_moveDown?id=%{id}">下移</s:a>		
					</s:else>
				</td>
			</tr>
        </s:iterator>
    ...
    <div id="TableTail">
        <div id="TableTail_inside">
            <s:a action="forum_addUI"><img src="${pageContext.request.contextPath}/style/images/createNew.png" /></s:a>
        </div>
    </div>
</div>
